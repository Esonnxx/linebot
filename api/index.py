...
def handle_day2(event):
    ...
    if event.message.text =="🗝平平淡淡才是真":
        ...
        text1 = "🗝仇愁得報"
        reply_arr.append(TextMessage(message_text))
        reply_arr.append(TextMessage(text1))
        line_bot_api.reply_message(
...
def handle_day3(event):
    ...
    if event.message.text =="🗝仇愁得報":
        ...
        msg_key = "🗝茫"
        reply_arr.append(TextMessage(message_text))
        reply_arr.append(TextMessage(msg_key))
        line_bot_api.reply_message(
            event.reply_token,reply_arr
        )
         # 發送圖片訊息
