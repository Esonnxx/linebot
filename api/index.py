...
def send_intro_and_question_day16(event):
    reply_arr = []

    # 顯示問題前的訊息
    intro_message = TextSendMessage(text="不知道現在的你能不能生氣呢？因為現在需要你回憶你前任~他最糟糕的時刻~")
...
def send_intro_and_question_day18(event):
    reply_arr = []

    # 顯示問題前的訊息
    intro_message = TextSendMessage(text="美好記憶當中應該也夾雜著那些被掩飾不堪的過往吧？")
...
def handle_day15(event):
    ...
    if user_message == "第十五天療程":
        ...
        day15_q1 = True
def handle_day16(event):
    user_message = event.message.text

    if user_message == "第十六天療程":
...
def process_day16_question2(event, line_bot_api):
    reply_arr = []
    text = "他甚麼時候完全不顧你的感受？"
    reply_arr.append(TextSendMessage(text))
    line_bot_api.reply_message(event.reply_token, reply_arr)
def process_day16_question3(event, line_bot_api):
    reply_arr = []
    text = "甚麼時候你生氣到想直接丟了他的東西？"
    reply_arr.append(TextSendMessage(text))
    line_bot_api.reply_message(event.reply_token, reply_arr)
def process_day16_key_reminder(event, line_bot_api):
    reply_arr = []
    text = "🗝悸動"
    reply_arr.append(TextSendMessage(text))
    line_bot_api.reply_message(event.reply_token, reply_arr)
...
def process_day18_question2(event, line_bot_api):
    reply_arr = []
    text = "你們第一次吵架是在哪裡？"
    reply_arr.append(TextSendMessage(text))
    line_bot_api.reply_message(event.reply_token, reply_arr)
def process_day18_question3(event, line_bot_api):
    reply_arr = []
    text = "他第一次讓你等他是在哪裡？"
    reply_arr.append(TextSendMessage(text))
    line_bot_api.reply_message(event.reply_token, reply_arr)
def process_day18_key_reminder(event, line_bot_api):
    reply_arr = []
    text = "🗝在乎"
    reply_arr.append(TextSendMessage(text))
    line_bot_api.reply_message(event.reply_token, reply_arr)
...
def handle_message(event):
    ...
    if event.message.text == "我同意":
        ...
        handle_day15(event)
        day15_q1 = True
    elif event.message.text == "第十六天療程":
        day16_state =True
        handle_day16(event)
        day16_q1 = True
        ...
        handle_day17(event)
        day17_q1 = True
    elif event.message.text == "第十八天療程":
        day18_state =True
        handle_day18(event)
        day18_q1 = True
    ...
    if day15_state and day15_q3:
        ...
        day15_q3 =False
        day15_state = False
    if day16_state and day16_q1:
        process_day16_question2(event, line_bot_api)
        day16_q1 =False
        day16_q2 =True
    if day16_state and day16_q2:
        process_day16_question3(event, line_bot_api)
        day16_q2 =False
        day16_q3 =True
    if day16_state and day16_q3:
    ...
    if day18_state and day18_q1:
        process_day18_question2(event, line_bot_api)
        day18_q1 =False
        day18_q2 =True
    if day18_state and day18_q2:
        process_day18_question3(event, line_bot_api)
        day18_q2 =False
        day18_q3 =True
    if day18_state and day18_q3:
